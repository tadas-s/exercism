...
pub fn tally(match_results: &str) -> String {
    ...
    totals_vec.sort_by_key(|team| std::cmp::Reverse(team.points()));
...
fn parse_line(l: &str) -> Option<(&str, Result, &str, Result)> {
    match &l.split(";").collect::<Vec<&str>>()[..] {
        [team_a, team_b, "win"] => Some((team_a, Result::Win, team_b, Result::Loss)),
        [team_a, team_b, "loss"] => Some((team_a, Result::Loss, team_b, Result::Win)),
        [team_a, team_b, "draw"] => Some((team_a, Result::Draw, team_b, Result::Draw)),
        _ => None,
    }
}
